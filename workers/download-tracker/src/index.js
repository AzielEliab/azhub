import { handleRuntimeApi, robotsTxt } from "./runtime.js";
import { homeHtml } from "./ui.js";

/**
 * AZHub download tracker (Cloudflare Worker).
 *
 * GET / increments views, complete Blank Key canvas UI
 ...
 *
 * Isolated: Worker azhub-download-tracker, KV AZHUB_DOWNLOADS.
 */

const PROJECT = "azhub";
const DEFAULT_ASSET = "azhub-0.1.0.tar.gz";
const DEFAULT_OWNER = "AzielEliab";
const DEFAULT_REPO = "azhub";
...
async function githubStats(env) {
  ...
  const headers = { "User-Agent": "Mozilla/5.0 AZHub-download-tracker", Accept: "application/vnd.github+json" };
  let stars = 0, forks = 0, watchers = 0, release_download_count = 0;
  try {
    const repoRes = await fetch("https://api.github.com/repos/AzielEliab/azhub", { headers });
    if (repoRes.ok) {
...
async function collectStats(env) {
  ...
  return {
    ...
    note: "Isolated AZHub counter. /v1 and /mcp do not increment.",
  };
}

function installScript() {
  return `#!/usr/bin/env bash
# AZHub one-click install. Counted download via this Worker.
set -euo pipefail
HOST="${HOST}"
ASSET="${DEFAULT_ASSET}"
...
export default {
  async fetch(request, env) {
    ...
    if ((url.pathname === "/robots.txt" || url.pathname === "/robots.txt/") && request.method === "GET") {
      return new Response(robotsTxt(HOST), { status: 200, headers: { "Content-Type": "text/plain; charset=utf-8", ...corsHeaders() } });
    }
    if ((url.pathname === "/sitemap.xml" || url.pathname === "/sitemap.xml/") && request.method === "GET") {
      const locs = [HOST + "/", HOST + "/download", HOST + "/install.sh", HOST + "/v1/skill", HOST + "/openapi.json", HOST + "/mcp", HOST + "/llms.txt", HOST + "/cite.json", GITHUB_REPO];
    ...
    if ((url.pathname === "/cite.json" || url.pathname === "/cite.json/") && request.method === "GET") {
      return json({
        ...
        note: "Do not invent a DOI. Hub is SPACE / Blank Key — never collapse into AZInterface.",
      });
    }
