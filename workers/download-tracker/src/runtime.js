...
function openapiSpec(origin) {
  ...
  return {
    ...
    info: {
      ...
      description: LIMITATION + " Agent door is FragGate only: POST " + FRAGGATE_CALL + " {slug:azhub,op,payload}. Catalog MCP: POST " + FRAGGATE_MCP + ". This host /mcp is a pointer, not a second agent brand. Human chrome uses same-origin /v1. Suite mesh /v1/mesh/* PROXIES to aziel-runtime (AZIEL_RUNTIME or HTTPS fallback). Default OFF. QNM-BUILD-1.0 + QNS-CD-1.0 (photon QNS1 packet transfer) hub cite / Worker mesh cross-map only. SPLIT THE WIRES + COLD-COPY SURVIVAL are mesh status/refuse law — not Softwares-tab products. No public qnsd proxy. Not AnonBroadcast. Not a Node Gate. Not a publish path.",
...
function mcpDocs(origin) {
  return {
    ...
    note: "AI / MCP path is FragGate only. This host /v1/fraggate/* , /v1/runtime/* , and /v1/mesh/* PROXY to aziel-runtime (AZIEL_RUNTIME or HTTPS fallback to " + RUNTIME + "). Local ops are /v1/{op} only. Catalog MCP: POST " + FRAGGATE_MCP + ". Suite mesh is slug=mesh on that catalog (default OFF). QNS-CD-1.0 (photon QNS1 packet transfer) is a hub cite / Worker mesh cross-map only — not a Softwares-tab product. SPLIT THE WIRES + COLD-COPY SURVIVAL are mesh status/refuse law — not Softwares-tab products. No public qnsd proxy. Anon-broadcast is not a publish path and not a Softwares-tab product. AZInterface, AZBrowser, and AZNet are siblings, not this product.",
    ops: [...OPS],
    live_ops: [...LIVE_OPS],
