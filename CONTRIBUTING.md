...
9. **Door vs local op.** `/v1/fraggate/*`, `/v1/runtime/*`, and
   ...
   + COLD-COPY SURVIVAL + RE-EXPAND-FROM-ARCHIVE are mesh
   status/refuse law (not Softwares-tab products). No public qnsd proxy. No Node Gate. No auto-heal.
...
- Suite mesh / QNS-CD-1.0 + SPLIT THE WIRES + COLD-COPY SURVIVAL + RE-EXPAND-FROM-ARCHIVE: `workers/download-tracker/src/mesh.js`
